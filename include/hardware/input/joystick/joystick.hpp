...
#define JOYSTICK_I2C_SPEED 100000

// LED颜色定义
#define JOYSTICK_LED_RED 0xFF0000
#define JOYSTICK_LED_GREEN 0x00FF00
#define JOYSTICK_LED_BLUE 0x0000FF
#define JOYSTICK_LED_OFF 0x000000
...
// 防抖配置结构
struct JoystickDebounceConfig {
    uint16_t threshold;
    uint16_t deadzone;
    uint8_t stable_count_required;
...
public:
    ...
    /**
     * @brief Update joystick state (for debouncing)
     */
    void update();
     ...
     */
    JoystickDirection get_stable_direction();
    ...
    /**
     * @brief Get Joystick x-axis and y-axis 16bits ADC values with timeout
     * @param adc_x pointer of x-axis ADC value
     * @param adc_y pointer of y-axis ADC value
     * @param timeout_ms timeout in milliseconds
     * @return number of bytes read, or negative value on error
     */
    int get_joy_adc_16bits_value_xy_with_timeout(uint16_t *adc_x, uint16_t *adc_y, uint32_t timeout_ms);
...
private:
    ...
    // 防抖相关成员变量
    JoystickDebounceConfig _debounce_config;
